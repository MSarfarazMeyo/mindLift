...
export const ratings = ['Poor', 'Fair', 'Neutral', 'Very Good', 'Excellent'] as const;
...
const WeeklyReportTab = () => {
    ...
    const calculateWeeklyStats = (): WeeklyStats => {
        const journalOnlyEntries = weeklyEntries.filter(entry =>
        ...
        const wellnessScores = questionEntries.map(entry => calculateWellnessScore(entry.notes));
        ...
        const moodTrend = moods.length > 0 ?
            (moods.length >= 3 ? 'Consistent tracking' : 'Limited data') : 'No mood data';
        ...
        const sleepAverage = sleepEntries.length > 0 ?
            `${sleepEntries.length} entries logged` : 'No sleep data';

        // Extract most common activities
        const allActivities = journalOnlyEntries
            .flatMap(entry => entry.activities ? entry.activities.split(',').map(a => a.trim()) : [])
    ...
    const { quote, loading } = useAlternativeQuoteAPI(weeklyStats.averageWellnessScore);

    const getIconByLevel = (level: any) => {
        switch (level) {
            case 'SAD':
                return { name: 'emoticon-sad-outline', lib: 'MaterialCommunityIcons', color: '#9CA3AF' };
            case 'LOW':
            ...
            case 'EXCELLENT':
                return { name: 'emoticon-excited-outline', lib: 'MaterialCommunityIcons', color: '#3B82F6' };
            default:
                return { name: 'help-outline', lib: 'MaterialIcons', color: '#9CA3AF' };
        }
    };

    const wellnessRating = weeklyStats.averageWellnessScore >= 4 ? 'Excellent' :
        weeklyStats.averageWellnessScore >= 3.5 ? 'Very Good' :
            weeklyStats.averageWellnessScore >= 3 ? 'Good' :
                weeklyStats.averageWellnessScore >= 2 ? 'Fair' : 'Poor';
        ...
        <ScrollView style={styles.container}>
            ...
            <View style={styles.wellnessContainer}>
                ...
                <View style={styles.circularProgress}>
                    <View style={[
                        ...
                        {
                            borderColor: wellnessRating === 'Excellent' ? '#2ecc71' :
                                wellnessRating === 'Very Good' ? '#27ae60' :
                                    wellnessRating === 'Good' ? '#f39c12' :
                                        wellnessRating === 'Fair' ? '#e67e22' : '#e74c3c'
                        }
                    ]}>
                        <Text style={styles.wellnessScore}>
                            {weeklyStats.averageWellnessScore > 0 ? weeklyStats.averageWellnessScore.toFixed(1) : 'N/A'}
                        </Text>
                        <Text style={[styles.wellnessRating, {
                            color:
                                wellnessRating === 'Excellent' ? '#2ecc71' :
                                    wellnessRating === 'Very Good' ? '#27ae60' :
                                        wellnessRating === 'Good' ? '#f39c12' :
                                            wellnessRating === 'Fair' ? '#e67e22' : '#e74c3c'
                        }]}>
            ...
            <View style={styles.summaryContainer}>
                ...
                <View style={styles.statsGrid}>
                    ...
                    <View style={styles.statCard}>
                        <MaterialCommunityIcons name="notebook-edit" size={24} color="#10B981" />
                    ...
                    <View style={styles.statCard}>
                        ...
                        <Text style={styles.statNumber}>{weeklyStats.questionsCompleted}</Text>
            ...
            <View style={styles.insightsContainer}>
                ...
                <View style={styles.insightItem}>
                    ...
                    <View style={styles.insightContent}>
                        ...
                        <Text style={styles.insightValue}>
                            {weeklyStats.weeklyStreak === 7 ? 'Perfect week! 🎉' :
                                weeklyStats.weeklyStreak >= 5 ? 'Great consistency!' :
                                    weeklyStats.weeklyStreak >= 3 ? 'Good progress' :
                ...
                <View style={styles.insightItem}>
                    ...
                    <View style={styles.insightContent}>
                        ...
                        <Text style={styles.insightValue}>
                            {weeklyStats.averageWellnessScore >= 4 ? 'Thriving this week!' :
                                weeklyStats.averageWellnessScore >= 3 ? 'Steady progress' :
                                    weeklyStats.averageWellnessScore >= 2 ? 'Building momentum' :
                                        'Focus on self-care'}
                        </Text>
                    </View>
                </View>
            ...
            {mostRecentQuestionEntry && (
                <View style={styles.chartContainer}>
                    <DailyQuestionsPieChart
                        questionResponses={parseQuestionResponses(mostRecentQuestionEntry.notes)}
                    />
                </View>
            )}
            ...
            <View style={styles.encouragementContainer}>
                <Text style={styles.encouragementTitle}>
                    {weeklyStats.weeklyStreak === 7 ? 'Perfect Week!' :
                        weeklyStats.weeklyStreak >= 5 ? 'Strong Week!' :
                            weeklyStats.weeklyStreak >= 3 ? 'Good Progress!' :
                                'Keep Building!'}
                </Text>
