...
import { Heart, RotateCcw, ArrowLeft } from 'lucide-react-native';
...
export default function ColorMatchGame() {
  ...
  const [targetColor, setTargetColor] = useState<ColorOption>(
    generateRandomColor(),
  );
  ...
  const startNewRound = useCallback(() => {
    ...
    const allColors = [...wrongColors, newTarget].sort(
      () => Math.random() - 0.5,
    );
  ...
  const handleColorPress = (color: ColorOption) => {
    ...
    if (color.rgb === targetColor.rgb) {
      ...
      const points = 5 * difficulty;
      const newScore = score + points;
      addGamePoints(20 + difficulty * 5);
      setScore(newScore);
    ...
    >
      ...
      <View style={styles.gameArea}>
        ...
        <Animated.View style={{ transform: [{ scale: targetScaleAnim }] }}>
          <View
            style={[styles.targetColor, { backgroundColor: targetColor.rgb }]}
          />
        </Animated.View>
