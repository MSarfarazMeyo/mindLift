...
export default function EndlessRunner() {
  ...
  const playerY = useRef<number>(SCREEN_HEIGHT - GROUND_HEIGHT - PLAYER_SIZE - 100);
  ...
  const gameLoop = useCallback(() => {
    ...
    if (obstacles.current.length === 0 || 
        obstacles.current[obstacles.current.length - 1].x < scrollOffset.current + SCREEN_WIDTH - OBSTACLE_GAP_MIN) {
  ...
  }, [gameStarted, gameOver, score, highScore, checkCollision, groundY, insets.top]);
            ...
            {Array.from({ length: 20 }).map((_, i) => (
              <View 
                ...
                style={[
                  ...
                  { 
                    left: (i * 60 - (scrollOffset.current % 60)) % (SCREEN_WIDTH + 60),
                  }
                ]}
              />
            ))}
